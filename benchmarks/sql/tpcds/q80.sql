...
FROM (
  SELECT 'store channel' AS channel, CONCAT('store', store_id) AS id, sales, returns_val, profit
  FROM ssr
  UNION ALL
  SELECT 'catalog channel' AS channel, CONCAT('catalog_page', catalog_page_id) AS id, sales, returns_val, profit
  FROM csr
  UNION ALL
  SELECT 'web channel' AS channel, CONCAT('web_site', web_site_id) AS id, sales, returns_val, profit
  FROM wsr
) x
