...
FROM (
  SELECT 'store channel' AS channel, CONCAT('store', s_store_id) AS id, sales, returns_val, (profit - profit_loss) AS profit
  FROM ssr
  UNION ALL
  SELECT 'catalog channel' AS channel, CONCAT('catalog_page', cp_catalog_page_id) AS id, sales, returns_val, (profit - profit_loss) AS profit
  FROM csr
  UNION ALL
  SELECT 'web channel' AS channel, CONCAT('web_site', web_site_id) AS id, sales, returns_val, (profit - profit_loss) AS profit
  FROM wsr
) x
