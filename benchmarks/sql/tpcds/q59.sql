...
FROM (
  ...
  WHERE d.d_week_seq = wss.d_week_seq AND ss_store_sk = s_store_sk AND d.d_year = 2000
) y,
(
  ...
  FROM wss, store, date_dim d
  WHERE d.d_week_seq = wss.d_week_seq AND ss_store_sk = s_store_sk AND d.d_year = 2000 + 1
) z
