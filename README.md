...
- **AI Agent in every driver** — The Quanton image ships an AI sidebar in the Spark UI itself: five tabs (Chat, Monitor, Diagnostics, **Savings**, Settings) that can answer plain-English questions about your running job, surface auto-detected health alerts, and break down compute waste as percentages of allocated executor-hours (never dollars). Your LLM API key stays in your browser. Optionally points at your Spark History Server to compare the current run against your historical baseline. Optional **await-termination** mode keeps the sidebar usable past job-end for post-mortem inspection. Enabled per-job via two `sparkConf` keys, or auto-injected when the operator is installed with `--set onehouseConfig.enableAIAgent=true`. See [`examples/tpcds-agent/`](examples/tpcds-agent/).
...
| `/run-tpcds-benchmark`   | Runs the TPC-DS read benchmark (99 queries on Parquet) comparing OSS Spark vs Quanton. Asks you for scale factor and configuration, gives live progress updates, and produces a per-query comparison table and chart. Optional: enables the in-driver **Spark Agent** sidebar (Chat / Recommendations / Diagnostics / Monitor / Cost / SHS cohort) on the Quanton run so you can interact with it live while the benchmark executes.                                       |
